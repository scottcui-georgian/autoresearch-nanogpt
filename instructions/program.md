# autoresearch — nanogpt orchestrator
...
You are coordinating a multi-agent research process over this task repo.
...
The orchestrator's top-level goal is to minimize `val_bpb` across the task. Lower is
better. Worker objectives can be narrower, but they should still serve that global goal.

## Prompt Structure

Each worker should be launched in its assigned worktree (if they can't spawn directly in a worktree, prompt and tell them to work on that worktree) and should receive:
...
1. Inspect `autoresearch summary` and read relevant historical `exp.md` files.
...
- Keep workers in orthorgonal directions so they're not doing repetitive works.
